$( document ).ready(function() {
    ...
    let bookTitleToSearch = "love"; // Que outros campos existem?
    $.getJSON(`https://www.googleapis.com/books/v1/volumes?q=${bookTitleToSearch}`, function(response){
        let books = [];
        ...
        let booksPerRow = 3;
        $("#content").append(`<div class='w3-container-w3-red'>
        <h1>Book Tinder</h1>
        <h6>Escolha o seu livro preferido!</h6></div>`);

        for(let currentBook = 0; currentBook != books.length; ++currentBook){            
            if(currentBook % booksPerRow === 0){
                $("#content").append("<div class='w3-row-padding'></div>");
            }
            let book = books[currentBook];
            
            $("#content > div.w3-row-padding:last").append(`<div class="w3-third"> 
                    <div  class="card" style="width: 20rem;">
                        <img class="card-img-top" src="${book.getImage()}" alt="${book.title}">
                        <div class="card-body">
                        <h5 class="card-title">${book.title}</h5>
                        <h6 class="card-subtitle mb-2 text-muted">${book.getAuthors()}<h6>
                        <p class="card-text"><small>${book.description != undefined ? book.description.substring(0,200) : "Não tinha descrição"}</small></p>
                        <div>
                            <span>  
                                <a href="#" class="btn btn-primary">Não gosto
                                ...
                                <a href="#" class="btn btn-primary">Gosto
